...
Dw = 150 # distance from the beach to the target (0 <= Dw <= 150)
...
display = 'comparison' # comparison or delta

# set up the plot for the given setting
if display == 'comparison': 
    plt.title(f'Times for Optimal Path and Straight-Line Path\nDs = {Ds} ft , Dw = {Dw} ft')
    plt.ylabel('Time (s)')
    plt.plot(ind,dep1,label='$T_{Optimal}$')
    plt.plot(ind,dep2,label='$T_{Straight}$')
elif display == 'delta':
	plt.title(f'Difference Between Times for Optimal Path and Straight-Line Path\nDs = {Ds} ft , Dw = {Dw} ft')
	plt.ylabel('∆T (s)')
	dep3 = []
